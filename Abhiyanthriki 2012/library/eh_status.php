...
function eh_status()

          {
               ...
               $result = mysql_query('select reg_status from events where name = \''.$event[1].'\'');
               check($result) ;
...
function eh_status_submit()
          {
              ...
              $bd = '';
               ...
               if($_POST['status'] == 'open')
                     {  
                         $result = mysql_query('update events set reg_status = 1 where name = \''.$event[1].'\'');
                         check($result) ;
                         $bd = 'Registration is now open for '.$event[1];
                      }
               elseif($_POST['status'] == 'close')
                       {
                          $result = mysql_query('update events set reg_status = 0 where name = \''.$event[1].'\'');
                          check($result) ;
