...
function admin_participants_submit()
            {
             ...
             if($type == 'Single')
                   {                        
                      ...
                      $bd = ' Registration for <b>'.$event.'</b><br/>
                              ...
                              <tr> <th> Sl. No.</th><th> Name </th><th> TK13 ID </th><th> Email ID </th><th> Mobile </th> </tr>' ;
                      while ( $row = mysql_fetch_row($result) )
                        {
                          $bd = $bd.'<tr><td>'.(++$i).'</td><td>'.idn($row[2]).'</td><td>'.$row[2].'</td><td>'.ide($row[2]).'</td><td>'.idp($row[2]).'</td></tr>' ;
                        }
                        $bd = $bd . '</tbody></table>';
                        return $bd;
                   }
             ...
             elseif ($type == 'Group') 
                    { 
                      ...
                      $bd = '<table cellspacing="0" cellpadding="1px"> <tbody><tr><th> Team </th> <th> TK13 ID </th><th> Name </th><th> Email ID </th><th> Mobile </th> </tr>' ;
                      $j = 0 ;
                      while ( $row = mysql_fetch_row($result) )
                        {
                          $temp = $row[2].','.$row[1];
